...
switch flag
    ...
    function sys=mdlOutputs(t,x,u)
        ...
        global U; % Access global variable U
        %U=[0];
        tic; % Start timer
        ...
        X = u(4);
        Y = u(5);
        %T_all = u(7); % Assuming T_all is the 7th input
        global x_dot;
        x_dot = u(8);
        %x_dot = 15;
        lf = 1.488;
        ...
        a = [1 - ((Ccf+Ccr) * T) / (1600), 0, -((lf*Ccf-lr*Ccr+m*x_dot^2) * T) / (1600*x_dot/2), 0, 0;
            0, 1, T, 0, 0;
            ((lf*Ccf-lr*Ccr) * T) / (I*x_dot/2), 0, 1-(2.5*Ccf*lf^2 + 2.5*Ccr*lr^2)*T/(I*x_dot/2*2.5*10), 0, 0;
        ...
        %Current_state = a * kesi(1:5,1) + b * kesi(6,1)
        ...
        error_1 = (Yita_ref - PSI * kesi - GAMMA * PHI);%/ 1.0e+55;
        %sub_from= PSI * kesi - GAMMA * PHI
        ...
        %cost = norm(error_1)
        %y_min_cons = -Ycmin + PSI * kesi + GAMMA * PHI
        ...
        %fprintf('The delta_Umin is %d \n',delta_Umin);
        %fprintf('M value is %d\n',M);
        % Solve quadratic programming problem A_cons, b_cons
